...
function getVersionList (cb) {
  ...
  https.get('https://shyftnetwork.github.io/shyft_solc-bin/bin/list.json', function (response) {
...
function downloadBinary (outputName, version, expectedHash) {
  ...
  https.get('https://shyftnetwork.github.io/shyft_solc-bin/bin/' + version, function (response) {
    if (response.statusCode !== 200) {
      console.log('Error downloading file: ' + response.statusCode);
      process.exit(1);
    }
